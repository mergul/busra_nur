...
class _ExhibitionBottomSheetState extends State<ExhibitionBottomSheet>
  ...
  double get maxHeight => 585.4285714285714;//MediaQuery.of(context).size.height;
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<UnmodifiableListView<NewsPayload>>(
      stream: widget.reactiveStreamBloc.topArticles,
                      ...
                      child: Stack(
                        children: <Widget>[
                          ...
                          for (Event event in events)
                            _buildFullItem(event, snapshot.data),
  ...
  Widget _buildFullItem(Event event, UnmodifiableListView<NewsPayload> data) {
    int index = events.indexOf(event);
    var _title = "Hello";
    var _date = "1.1.1";
    if (data.length >= index+1) {
      _title = data.elementAt(index).topic;
      _date = data.elementAt(index).date.toIso8601String();
    }
